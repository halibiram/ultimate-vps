...
export class SSHService {
   ...
   */
  async createSSHUser(username: string, password: string): Promise<boolean> {
    try {
   ...
   */
  async deleteSSHUser(username: string): Promise<boolean> {
    try {
   ...
   */
  async toggleUserLock(username: string, lock: boolean): Promise<boolean> {
   ...
   */
  async getUserConnections(username: string): Promise<number> {
    try {
  ...
  async getActiveConnections(): Promise<any[]> {
    try {
      ...
      return [
        { service: 'SSH-22', port: 22, connections: parseInt(port22.trim()) - 1 }, // Subtract 1 for the header line
